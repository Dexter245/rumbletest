...
int main(int argc, char **argv)
{
    ...
    while(appletMainLoop())
    {
        ...
        if (R_SUCCEEDED(rc) && (kHeld & KEY_R))
        {
            ...
            consoleClear();
            if (kDown & KEY_X) VibrationValue.amp_low   += 0.1f;
            if (kDown & KEY_B) VibrationValue.amp_low   -= 0.1f;
            if (kDown & KEY_A) VibrationValue.freq_low += 12.0f;
            if (kDown & KEY_Y) VibrationValue.freq_low -= 12.0f;
            printf("low amp: %.1f, low freq: %.1f\n", VibrationValue.amp_low, VibrationValue.freq_low);

            if (kDown & KEY_UP) VibrationValue.amp_high   += 0.1f;
            if (kDown & KEY_DOWN) VibrationValue.amp_high   -= 0.1f;
            if (kDown & KEY_RIGHT) VibrationValue.freq_high += 12.0f;
            if (kDown & KEY_LEFT) VibrationValue.freq_high -= 12.0f;
            printf("high amp: %.1f, high freq: %.1f\n", VibrationValue.amp_high, VibrationValue.freq_high);
